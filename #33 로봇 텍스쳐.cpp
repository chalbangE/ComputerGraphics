...
GLvoid drawScene()
{
	...
	Floor.draw_prepare(false, "Texture");
	Floor.draw_prepare(TexorColorLocation, "Texture_bool");
	glUniform1f(DistanceLocation, distance(Light.pos, Floor.pos));
	Floor.draw("solid");

	for (int i = 0; i < Cube.size(); ++i) {
		Cube[i].Update();
		Cube[i].draw_prepare(PosLocation, "Pos");
		Cube[i].draw_prepare(WorldTransLocation, "World");
		Cube[i].draw_prepare(NormalLocation, "Normal");
		Cube[i].draw_prepare(UvLocation, "UV");
		Cube[i].draw_prepare(false, "Texture");
		Cube[i].draw_prepare(TexorColorLocation, "Texture_bool");
		glUniform1f(DistanceLocation, distance(Light.pos, Cube[i].pos));
		Cube[i].draw("solid");
	}
...
void Keyboard(unsigned char key, int x, int y)
{
	...
	case 'i':
	case 'I': {
		Object[mod].rotate_theta.y = 180.f;
		Object[mod].pos.z += -0.01f;
		break;
	}
	case 'k':
	case 'K': {
		Object[mod].rotate_theta.y = 0.f;
		Object[mod].pos.z += 0.01f;
		break;
	}
	case 'j':
	case 'J': {
		Object[mod].rotate_theta.y = 270.f;
		Object[mod].pos.x += -0.01f;
		break;
	}
	case 'l':
	case 'L': {
		Object[mod].rotate_theta.y = 90.f;
		Object[mod].pos.x += 0.01f;
		break;
	}
	case 'c':
...
void Init()
{
	...
	{
		{
			...
			Object[0].scale = glm::vec3{ 0.2f, 0.2f, 0.2f };
			Object[0].midpos = glm::vec3{ 0.f, 0.f, 0.f };
		...
		}
		{
			...
			Object[1].midpos = glm::vec3{ 0.f, 0.f, 0.f };
			Object[1].scale = glm::vec3{ 0.2f, 0.2f, 0.2f };
	...
	// Floor
	{
		...
		Floor.scale = glm::vec3{ 3.f, 0.02f, 3.f };

		Floor.imgLoad("./IMG/체크무늬.png");
	}
	...
	// 장애물
	{
		...
		if (inputFile.is_open())
			Cube.back().objLoad(inputFile);
		else
			std::cerr << "Failed to obj file" << std::endl;

		Cube.back().scale = glm::vec3{ 0.2f, 0.2f, 0.2f };
		Cube.back().pos = glm::vec3{ 0.5f, 0.25 * Cube.back().scale.y, 0.5f };

		Cube.back().imgLoad("./IMG/반투명보라.png");
	}
	{
		std::ifstream inputFile("./OBJ/cube_tex.obj");
		Cube.emplace_back();
		...
		Cube.back().scale = glm::vec3{ 0.1f, 1.f, 0.1f };
		Cube.back().pos = glm::vec3{ -0.5f, 0.25 * Cube.back().scale.y, -0.5f };

		Cube.back().imgLoad("./IMG/반투명파랑.png");
	}
	{
		std::ifstream inputFile("./OBJ/cube_tex.obj");
		Cube.emplace_back();
		...
		Cube.back().scale = glm::vec3{ 0.1f, 1.f, 0.1f };
		Cube.back().pos = glm::vec3{ -0.3f, 0.25 * Cube.back().scale.y, -0.5f };
	...
	}
	{
		...
		Cube.back().scale = glm::vec3{ 0.4f, 0.1f, 0.1f };
		Cube.back().pos = glm::vec3{ -0.4f, 0.5f, -0.5f };
